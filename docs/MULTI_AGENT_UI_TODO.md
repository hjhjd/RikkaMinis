...
- [x] 删除/归档采用保守策略：默认 Agent 受保护；其他 Agent 归档前展示话题数并迁移话题到默认 Agent；头像和记忆暂不物理删除。
...
- [x] 默认 Agent 不可删除/归档。
- [x] 有历史话题的 Agent 不做无提示级联删除。
- [x] 归档时将话题迁移到默认 Agent；归档作为当前安全默认方案。
- [x] 记忆与头像不随归档物理清理，避免破坏恢复路径。
