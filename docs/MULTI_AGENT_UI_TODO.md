...
- [ ] `ChatViewModel` 加载 session 时同时加载绑定 Agent。
- [ ] 草稿 session id 旁保存/传递稳定 `agentId`；草稿提升为真实 session 时不丢绑定。
- [ ] `buildSystemPrompt()` 改为显式接收/解析 Agent 配置。
- [ ] 按“平台模板 → Agent 指令 → 技能/MCP → Agent 记忆 → runtime”固定装配顺序。
- [ ] 技能 fragment 改用 agent + session 两层解析。
...
- [ ] 话题卡片增加 Agent 头像/名称或颜色标识，避免聚合列表中归属不明。
- [ ] 搜索同时匹配标题、最后消息及 Agent 名称；大数据量时沿用数据库查询而非全量 Compose 过滤。
- [ ] “新建话题”绑定当前 Agent。
- [ ] 保留长按删除、置顶和当前话题高亮。
...
- [ ] 从 `SettingsScreen` 的 Agent Runtime 区移除：
  - 技能
  - 人格
  - 系统提示词
  - 记忆
- [ ] “人格 + 系统提示词”在 Agent 配置中合并为一个入口，不保留两个并列页面。
- [ ] 全局设置保留 Provider、模型组、MCP、环境变量、沙箱、权限、外观、备份等跨 Agent 能力。
