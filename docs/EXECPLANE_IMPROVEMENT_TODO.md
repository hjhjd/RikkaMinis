...
> 创建日期：2026-08-13
>
> 状态：核心施工完成，进入维护期（阶段 1–4 已完成并联调，2026-08-13）
>
> 原则：先修正确性与数据安全，再扩展协议能力；每批改动都必须有回归测试。

## 1. 当前结论与发布边界

阶段 1–4 的核心正确性、资源保护、事务传输和 ExecPlane 0.2 已完成，并通过 Android 单测/打包、Python 回归以及正向和反向真机联调。个人设备、可信网络、独立低权限 Linux 用户或容器环境可以继续使用，当前没有必须立即施工的功能阶段。

剩余任务按发布条件管理：

- **扩大到不可信远程网络、多人共享或自动调度前必须完成**：T5.2 信任策略、T5.3 审计日志、T6 自动端到端与故障注入；
- **高对抗同机文件系统场景前必须完成**：T3.4 长期项 `openat/dir_fd/O_NOFOLLOW`；
- **维护性改进，可按需施工**：T0.2 测试目录整理、统一验证脚本、设置页补充风险说明；
- T5.1 常量时间 Token 比较、最短长度、日志脱敏和 1008 关闭码已经实现，只剩专项测试与文档状态同步。

面向使用者和维护者的总览见 [`ANDROID_WS_SANDBOX.md`](./ANDROID_WS_SANDBOX.md)，执行端部署细节见 [`../tools/execplane/README.md`](../tools/execplane/README.md)。
...
1. T5.2：反向执行端默认 `RESTRICTED`，用户显式提升信任等级；
2. T5.3：记录脱敏执行审计；
3. T6.1/T6.2：自动端到端、断线、commit 响应丢失、磁盘不足和网络切换故障注入；
4. T6.3：将 Android、Python 和真机验收固化为发布门禁。

## 仅高对抗场景必须

1. 使用 `openat/dir_fd/O_NOFOLLOW` 逐级解析 Linux 文件路径；
2. 消除路径校验与实际打开之间剩余的 TOCTOU 窗口；
3. 结合容器 seccomp、只读挂载和独立低权限用户形成真正 OS 级隔离。

## 当前可延期

1. Python 测试目录重组和统一脚本；
2. 设置页更完整的 TLS、文件系统不共享和环境变量风险说明；
3. 审计查看 UI、筛选和导出；
4. 更复杂的自动调度和信任排序。

维护期原则：除安全修复外，不再扩展 0.2 wire protocol；下一次协议变更必须升级版本并同步 Kotlin、Python、共享向量和文档。