#!/usr/bin/env python3
"""ExecPlane reverse agent with bounded concurrent requests."""
...
async def process_request(ws,raw,runtime,send_lock):
 ...
 async with send_lock: await ws.send(json.dumps(response,separators=(",",":")))

async def run(uri,token,name,runtime):
 delay=1; runtime.transfers.start()
 try:
  while True:
   tasks=set()
   try:
...
async def main():
 ...
 await run(a.uri,a.token,a.name,Runtime(a.allow_root,max_exec=a.max_exec,transfer_options={'temp_limit':a.temp_limit_mb*1024*1024,'max_active':a.max_transfers}))
