...
from runtime import Runtime, RpcFault, CAPS, PROTOCOL_VERSION
  ...
  while True:
   ...
   try:
    async with connect(uri,additional_headers={'X-Minis-Token':token},ping_interval=5,ping_timeout=15,max_size=2*1024*1024) as ws:
     limits=runtime.capabilities()['limits'];params={'protocol':PROTOCOL_VERSION,'name':name,'caps':sorted(CAPS),'resources':{'cpuCores':os.cpu_count(),'os':platform.system().lower(),'arch':platform.machine()},'limits':limits,'trust':'RESTRICTED','tags':['reverse']}
...
async def main():
 ap=argparse.ArgumentParser();ap.add_argument('--uri',default='ws://127.0.0.1:8765');ap.add_argument('--token',default=os.getenv('EXECPLANE_TOKEN'));ap.add_argument('--name',default='proot-test');ap.add_argument('--allow-root',action='append',required=True);ap.add_argument('--max-exec',type=int,default=256);ap.add_argument('--temp-limit-mb',type=int,default=1024);ap.add_argument('--max-transfers',type=int,default=8);a=ap.parse_args()
 if not a.token or len(a.token)<32:ap.error('provide a token of at least 32 characters')
 await run(a.uri,a.token,a.name,Runtime(a.allow_root,max_exec=a.max_exec,transfer_options={'temp_limit':a.temp_limit_mb*1024*1024,'max_active':a.max_transfers},name=a.name))
