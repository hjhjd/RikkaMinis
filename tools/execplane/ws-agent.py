  ...
  while True:
   ...
   try:
    async with connect(uri,additional_headers={'X-Minis-Token':token},ping_interval=5,ping_timeout=15,max_size=2*1024*1024) as ws:
     limits=runtime.capabilities()['limits'];params={'protocol':PROTOCOL_VERSION,'name':name,'caps':sorted(CAPS),'resources':{'cpuCores':os.cpu_count(),'os':platform.system().lower(),'arch':platform.machine()},'limits':limits,'trust':'RESTRICTED','tags':['reverse']};instruction_set=runtime.capabilities().get('instructionSet');params.update({'instructionSet':instruction_set} if instruction_set else {})
...
async def main():
 ...
 await run(a.uri,a.token,a.name,Runtime(a.allow_root,max_exec=a.max_exec,transfer_options={'temp_limit':a.temp_limit_mb*1024*1024,'max_active':a.max_transfers},name=a.name,instruction_set=load_instruction_set(a.instructions),dispatch_handler=load_dispatch_handler(a.dispatch_plugin)))
